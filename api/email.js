// api/email.js - Envío de emails via Brevo (ex Sendinblue) - gratis 300/día
...
export default async function handler(req, res) {
  ...
  if (!BREVO_API_KEY) return res.status(500).json({ ok: false, error: 'BREVO_API_KEY no configurada' });
...
<body style="margin:0;padding:0;background:#f0eee8;font-family:-apple-system,Helvetica Neue,Arial,sans-serif;">
  <div style="max-width:600px;margin:24px auto;background:#fff;border-radius:12px;overflow:hidden;">
    ...
    <div style="background:#f8f7f3;border-bottom:1px solid #e8e6e0;padding:16px 32px;display:flex;gap:24px;">
      <div style="text-align:center;margin-right:16px">
        <div style="font-size:22px;font-weight:800;color:#DF1278;">${d.distribucion ? d.distribucion.delete_count : '—'}</div>
        <div style="font-size:11px;color:#888;text-transform:uppercase;letter-spacing:0.05em;">Delete</div>
      </div>
      <div style="text-align:center;margin-right:16px">
        <div style="font-size:22px;font-weight:800;color:#04ACA7;">${d.distribucion ? d.distribucion.keep_count : '—'}</div>
        <div style="font-size:11px;color:#888;text-transform:uppercase;letter-spacing:0.05em;">Keep</div>
      </div>
      <div style="text-align:center;margin-right:16px">
        <div style="font-size:22px;font-weight:800;color:#7B5CFA;">${d.distribucion ? d.distribucion.create_count : '—'}</div>
    ...
    <div style="padding:28px 32px;">
      ...
      <div style="margin-bottom:24px;">
        ...
        ${d.ideas_radicales.map(i => `
          <div style="border:1px solid #e8e6e0;border-radius:8px;padding:14px 16px;margin-bottom:8px;">
            <div style="font-size:14px;font-weight:700;color:#2C2C2A;margin-bottom:4px;">${i.titulo}
              <span style="font-size:10px;font-weight:700;padding:2px 8px;border-radius:20px;margin-left:6px;background:${i.factibilidad==='alta'?'#e8f5e9;color:#2e7d32':i.factibilidad==='media'?'#fff3e0;color:#e65100':'#fce4ec;color:#c62828'};">${i.factibilidad}</span>
            </div>
      ...
      <div style="margin-bottom:24px;">
        <div style="font-size:11px;font-weight:700;letter-spacing:0.1em;color:#888;text-transform:uppercase;margin-bottom:12px;">🎯 Oportunidades</div>
        ${d.oportunidades.map(o => `
          <div style="border:1px solid #e8e6e0;border-radius:8px;padding:14px 16px;margin-bottom:8px;">
            <div style="font-size:14px;font-weight:700;color:#2C2C2A;margin-bottom:4px;">${o.titulo}</div>
            <div style="font-size:13px;color:#5f5e5a;line-height:1.5;">${o.descripcion}</div>
          </div>
        `).join('')}
      </div>` : ''}
      ...
      <div style="margin-bottom:24px;">
        ...
        ${d.proximos_pasos.map((p, i) => `
          <div style="display:flex;gap:12px;padding:10px 0;border-bottom:1px solid #f0eee8;">
            <div style="width:22px;height:22px;border-radius:50%;background:#534AB7;color:#fff;font-size:11px;font-weight:700;text-align:center;line-height:22px;flex-shrink:0;">${i+1}</div>
            <div>
    ...
    <div style="background:#f8f7f3;padding:20px 32px;text-align:center;border-top:1px solid #e8e6e0;">
      <div style="font-size:12px;color:#aaa;">Generado durante el SAP Partner Summit · Delete / Keep / Create · 2025</div>
    </div>
  </div>
</body>
</html>`;
  ...
  try {
    ...
    if (emailData.messageId) {
      return res.json({ ok: true, id: emailData.messageId });
    } else {
      return res.status(400).json({ ok: false, error: emailData.message || JSON.stringify(emailData) });
    }
  } catch (e) {
